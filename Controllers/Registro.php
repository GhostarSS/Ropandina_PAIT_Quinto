    ...
    public function save()
{
    if (isset($_POST['nombre'], $_POST['email'], $_POST['clave'])) {
        if (empty($_POST['nombre']) || empty($_POST['email']) || empty($_POST['clave'])) {
            ...
            // Limpiando los datos de entrada
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                $respuesta = ['msg' => 'El nombre debe tener más de 5 dígitos', 'icono' => 'warning'];
            } elseif (strlen($clave) < 8) {
                $respuesta = ['msg' => 'La contraseña debe tener más de 8 dígitos', 'icono' => 'warning'];
            } else {
                ...
                if (empty($consulta)) {
                    $data = $this->model->registrar($email, $nombre, $claveHash, $tipo);
                    if ($data > 0) {
